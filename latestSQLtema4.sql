...
/*
Crea un procedimiento llamado subirSalario que seleccione el salario del número de empleado
que se le pasa por parámetro y que le aumente el salario en un incremento que también se le
pasa por parámetro. Si el salario es nulo, el salario se fijará al valor del incremento. Habrá una
excepción, para cuando no exista el empleado, mostrando un mensaje.
*/

create or replace procedure subirSalario (codEmple emple.emp_no%type, incremento number) as
    vsalarioForEmple emple.salario%type;
    sinSalario exception;
    noHayEmpleado exception;
    
    begin
    select salario into vsalarioforemple
    from emple
    where codEmple = emp_no;

    
    if vsalarioforemple is null OR vsalarioforEmple = 0 then
    raise sinSalario;
    end if;
    
    dbms_output.put_line('Salario subido ' || incremento || ' para empleado con código ' || codEmple);
    update emple 
        set salario = salario + incremento
        where emp_no = codEmple;

    exception
        when no_data_found then
            dbms_output.put_line('No existe este empleado');
        when sinSalario then
            dbms_output.put_line('Este empleado no tiene un salario que subir');
end;
/
    
execute subirSalario(99,199);

/*
Modifica el procedimiento anterior subirSalario lanzando la excepción con
RAISE_APPLICATION_ERROR, con el número –20010 cuando el salario sea nulo.
*/

insert into emple (emp_no, salario, dept_no) values (101, null, 10);

create or replace procedure subirSalario (codEmple emple.emp_no%type, incremento number) as
    vsalarioForEmple emple.salario%type;
    sinSalario exception;
    salarioNull exception;
    noHayEmpleado exception;
    ...
    if vsalarioforemple is null then
    raise salarioNull;
    end if;
    if vsalarioforEmple = 0 then
    raise sinSalario;
    end if;
    ...
    exception
        when salarioNull then
            RAISE_APPLICATION_ERROR(-20010, 'Salario es null');
        when no_data_found then
            dbms_output.put_line('No existe este empleado');
        when sinSalario then
            dbms_output.put_line('Este empleado no tiene un salario que subir');
...
execute subirSalario(101,1023);

/* 
Escribe el conjunto de procedimientos detallados a continuación:
8.1. insert_depart: permite insertar un departamento nuevo. El procedimiento recibe el nombre
y la localidad del nuevo departamento. Creará el nuevo departamento comprobando que el
nombre no se duplique (si se duplica no hay que hacer la inserción) y le asignará como número
de departamento el máximo + 10. Si la tabla es vacía el primer número de departamento es 10.
8.2. borrar_depart: permite borrar un departamento. El procedimiento recibirá dos números de
departamento de los cuales el primero corresponde al departamento que queremos borrar y el
segundo al departamento al que pasarán los empleados del departamento que se va eliminar.
El procedimiento se encargará de realizar los cambios oportunos en los números de
departamento de los empleados correspondientes. Si el departamento donde se van a pasar los
empleados no existe el procedimiento no tiene que hacer nada.
8.3. modifica_localidad: modifica la localidad del departamento. El procedimiento recibirá el
número del departamento a modificar y la nueva localidad, y realizará el cambio solicitado.
8.4. visualiza_datos_depart: visualizará los datos de un departamento cuyo número se pasará
en la llamada. Además de los datos relativos al departamento, se visualizará el nombre del
departamento, localidad y número de empleados que pertenecen actualmente al
departamento
*/

create or replace procedure insert_depart(newDep depart.dnombre%type, newLoc depart.loc%type) as
    
    nombreExists number;
    departExiste exception;
    maxDeptNo depart.dept_no%type;
    begin
    
    select count(dnombre) into nombreExists
        from depart 
        where dnombre = newDep;
    
    select max(dept_no) into maxDeptNo
    from depart;
    
    if (nombreExists) > 0 then
        raise departExiste;
    end if;
            
    insert into depart (dept_no, dnombre, loc) values
    (maxDeptNo + 10, UPPER(newDep), UPPER(newLoc)); 
    
    exception
        when departExiste then
            RAISE_APPLICATION_ERROR(-20011, 'El departamento ya existe');
    end;
    /
    
execute insert_depart('Jamflex', 'VARLAMORE');
execute insert_depart('innkeeper', 'Liscor');
